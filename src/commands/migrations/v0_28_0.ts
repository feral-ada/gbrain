 ...
 *   A. Schema     — verify migrations v31 + v32 already applied (the schema
  ...
  try {
    ...
    if (v < 32) {
      return {
        name: 'schema',
        status: 'failed',
        detail: `expected schema version >= 32 (takes + access_tokens.permissions); got ${v}. Run \`gbrain apply-migrations --yes\` to apply.`,
      };
    }
    ...
    return { name: 'schema', status: 'complete', detail: 'schema v32 applied; takes + synthesis_evidence present' };
  } catch (e) {
