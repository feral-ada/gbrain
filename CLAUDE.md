...
### BrainBench — public benchmark for personal-knowledge agent stacks (v0.15+)

- `eval/README.md` — BrainBench entry point; explains the 4-adapter scorecard and contributor paths
- `eval/runner/multi-adapter.ts` — Side-by-side runner. Scores 4 adapters (gbrain-after, ripgrep-bm25, vector-only, hybrid-nograph) on the same corpus + queries. `bun run eval:run` (N=5) or `eval:run:dev` (N=1). Produces a scorecard with tolerance bands.
- `eval/runner/types.ts` — Canonical adapter contract: `Adapter.init(rawPages, config) → BrainState`, `Adapter.query(q, state) → RankedDoc[]`. `PublicPage` / `PublicQuery` (sealed-qrels, v1 Complete) land here. `Page.type` enum includes `email | slack | calendar-event | note` for amara-life-v1.
- `eval/runner/adapters/` — Reference adapters. ripgrep-bm25 (classic IR baseline), vector-only (commodity cosine RAG, same embedder as gbrain), hybrid-nograph (gbrain minus graph layer — same codebase, graph disabled, for A/B isolation). gbrain-after wrapper is inlined in multi-adapter.ts.
- `eval/runner/queries/` — Query bank. tier5-fuzzy.ts (30 vague-recall probes, hand-authored), tier5_5-synthetic.ts (50 AI-authored outsider probes labeled with provenance), validator.ts (schema enforcement + temporal `as_of_date` rule; slug regex requires exactly one slash).
- `eval/runner/type-accuracy.ts` — Per-link-type accuracy (Cat 2) on the 240-page rich-prose corpus. Cat 2 is where extractor regressions show up.
- `eval/runner/adversarial.ts` — Cat 10 robustness suite (22 hand-crafted cases: code-fence leaks, unicode, substring FPs). Extended at scale by Cat 6 when it ships.
- `eval/runner/all.ts` — Master runner. Currently sequential execSync; v1 Complete Day 10 rewrites to in-process async with `p-limit(2)` PGLite concurrency cap + shared LLM rate bucket.
- `eval/generators/world.ts` + `gen.ts` — World-v1 scaffolder + Opus prose generator (240 pages: 80 people, 80 companies, 50 meetings, 30 concepts). Committed, regenerable from seed, cost-gated at $80.
- `eval/generators/world-html.ts` — Contributor-facing explorer for the fictional corpus (`bun run eval:world:view`).
- `eval/generators/amara-life.ts` — Amara Okafor fictional-life skeleton (v0.15+). Procedural, Mulberry32-seeded. 15 contacts picked from world-v1 + 50 emails + 300 Slack msgs + 20 calendar events + 8 meeting transcripts + 40 first-person notes. Plants 10 contradictions + 5 stale facts + 5 poison items + 3 implicit preferences at deterministic positions; fixture_ids cross-reference gold files.
- `eval/generators/amara-life-gen.ts` — Opus prose expansion of the amara-life skeleton (v0.15+). Structured cache key = `sha256({schema_version, template_id, template_hash, model_id, model_params, seed, item_spec_hash})`. Cost-gated at $20 hard-stop. `--dry-run` for smoke testing without LLM spend. Run via `bun run eval:generate-amara-life`.
- `eval/schemas/*.schema.json` (v0.15+) — Portable JSON schemas pinning the v1→v2 Inspect AI bridge contract. 6 schemas: corpus-manifest, public-probe (PublicQuery with gold stripped), tool-schema (12 read + 3 dry_run tools, 32K tool-output cap), transcript, scorecard (N ∈ {1, 5, 10}), evidence-contract (structured judge input — raw tool output never reaches judge).
- `eval/data/world-v1/` — Committed 240-page canonical corpus (JSON shards + `_ledger.json`). Each page carries `_facts` gold metadata (never passed to adapters; sealed at the boundary).
- `eval/data/gold/*.json` (v0.15+) — Sealed qrels + adversarial perturbation gold: entities, backlinks, qrels, contradictions, poison, personalization-rubric, implicit-preferences, citations. Empty-but-valid templates in v0.15; Day 3b hand-authors real content against the generated amara-life-v1 corpus.
- `eval/data/amara-life-v1/` — Generated by `eval/generators/amara-life-gen.ts`. Shape: `inbox/emails.jsonl`, `slack/messages.jsonl`, `calendar.ics`, `meetings/*.md`, `notes/*.md`, `docs/*.md`, `corpus-manifest.json`. Not committed in v0.15 (user runs `eval:generate-amara-life` first).
- `docs/benchmarks/*.md` — Committed benchmark reports. Latest: `2026-04-19-brainbench-multi-adapter.md` (4-adapter scorecard), `2026-04-19-brainbench-v0_11-vs-v0_12.md` (regression comparison isolating v0.12 extract contribution).
- `test/eval/*.test.ts` (v0.15+) — BrainBench foundation tests: schemas.test.ts (48, schema round-trip + coherence), amara-life.test.ts (17, skeleton determinism + perturbation counts + slug regex), amara-life-gen.test.ts (10, structured cache-key invalidation).
...
`test/eval/schemas.test.ts` (v0.15 BrainBench portable JSON schemas — round-trip, $schema/$id/title, Page.type enum coherence, 12+3 tool pinning, N ∈ {1,5,10} enum),
`test/eval/amara-life.test.ts` (v0.15 amara-life skeleton — counts 50/300/20/8/40, perturbations 10/5/5/3, Mulberry32 seed determinism, slug regex conformance, unique fixture_ids, amara-on-every-email invariant),
`test/eval/amara-life-gen.test.ts` (v0.15 Opus generator structured cache key per codex fix #18 — every key field invalidates cleanly, canonical JSON stable under reorder, per-item spec-hash uniqueness).
